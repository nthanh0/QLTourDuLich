...
namespace QLTourDuLich.Services
{
    public class BookingService
    {
        ...
        // DEBUG: Lấy tất cả giá trị Status DISTINCT
        public async Task<List<string>> GetDistinctStatusesAsync()
        {
            var query = "SELECT DISTINCT Status FROM Bookings WHERE Status IS NOT NULL ORDER BY Status";
            return await _dbService.ExecuteQueryAsync(query, reader => reader.GetString(0));
        }
